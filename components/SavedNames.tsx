...
export const SavedNames: React.FC<SavedNamesProps> = ({ savedNames, removeSavedName, petGender }) => {
    ...
    const handleDownloadTextList = () => {
        ...
        const footer = t.saved_names.text_list_footer || "Generated by NameMyPet.org";
        ...
        link.href = url;
        link.download = 'my-pet-names.txt';
        ...
        <Card>
                ...
                <>
                    ...
                    <div className="flex flex-col sm:flex-row justify-center gap-3 pt-2 border-t border-white/10">
                         ...
                         <div className="flex gap-2 w-full sm:w-auto">
                            <Button onClick={handleCopyList} variant="secondary" className="!py-3 !px-4 flex-1 sm:flex-auto" title="Copy to Clipboard">
                                <CopyIcon className="w-5 h-5" />
                                {copied ? t.saved_names.btn_copied : t.saved_names.btn_copy}
                            </Button>
                            <Button onClick={handleDownloadTextList} variant="secondary" className="!py-3 !px-4 flex-1 sm:flex-auto" title="Download Text List">
            ...
            {showImageCreator && (
                <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/50 backdrop-blur-md">
                    {/* Changed items-start to items-center for better centering, flex layout updated */}
                    <div className="flex min-h-full items-center justify-center p-0 sm:p-4 text-center">
                        <div className="relative w-full max-w-7xl transform bg-[var(--card-bg)] backdrop-blur-xl text-left shadow-2xl transition-all sm:rounded-2xl max-h-screen sm:max-h-[95vh] flex flex-col overflow-hidden border border-white/20">
                            
                            {/* Sticky Header - Updated color to match Card theme */}
                            ...
                            <div className="overflow-y-auto flex-grow p-4 sm:p-8">
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 pb-20 sm:pb-0 h-full">
                                     {/* Controls - Updated background to match Card/Glassmorphism theme */}
                                     <div className="space-y-6 order-2 lg:order-1 flex flex-col justify-center">
                                        ...
                                        <div className="pt-6 border-t border-[var(--text-main)]/10">
                                            ...
                                            <div className="flex flex-col gap-4">
                                                <Button onClick={handleDownloadImage} disabled={isDownloading} variant="secondary" className="!py-4 !text-lg">
                                                    <DownloadIcon className="w-6 h-6 mr-2" />
                                                    {isDownloading ? t.saved_names.btn_saving : t.saved_names.btn_download}
                                                </Button>
                                                <Button onClick={handleShare} disabled={isSharing} variant="primary" className="!py-4 !text-lg">
                                     ...
                                     {/* Preview - Order 2 on Mobile (top), Order 2 on Desktop (right) */}
                                     {/* Applied scaling to ensure it fits on screen without scrolling on most laptops */}
                                     <div className="flex flex-col items-center justify-start lg:justify-center order-1 lg:order-2 min-h-[500px]">
                                        <div className="w-full flex justify-center overflow-visible py-4 md:py-0">
                                            <div 
                                                className="cursor-grab active:cursor-grabbing transform origin-top lg:origin-center transition-transform duration-300 scale-[0.65] sm:scale-[0.80] md:scale-90 xl:scale-100"
