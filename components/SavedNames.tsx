...
const ShareIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M7.217 10.907a2.25 2.25 0 1 0 0 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186l9.566-5.314m-9.566 7.5l9.566 5.314m0 0a2.25 2.25 0 1 0 3.935 2.186a2.25 2.25 0 0 0-3.935-2.186Zm0-12.814a2.25 2.25 0 1 0 3.933-2.186 2.25 2.25 0 0 0-3.933 2.186Z" /></svg>
);
...
export const SavedNames: React.FC<SavedNamesProps> = ({ savedNames, removeSavedName, petGender }) => {
    ...
    const handleDownloadImage = async () => {
        ...
        try {
             ...
             const blob = await toBlob(cardRef.current, getSnapshotOptions());
    ...
    const handleShare = async () => {
        ...
        try {
            // Wait for paint cycle before capturing
            await new Promise(resolve => setTimeout(resolve, 1200)); 
            const blob = await toBlob(cardRef.current, getSnapshotOptions());
            if (blob) {
                const file = new File([blob], 'MyPet.png', { type: 'image/png' });
                if (navigator.canShare && navigator.canShare({ files: [file] })) {
                    await navigator.share({ files: [file], title: t.share_texts.card_title, text: t.share_texts.card_body });
                } else {
                    setActionError("Sharing not supported on this browser.");
                }
            }
        } catch (error: any) {
        ...
        <Card>
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-4">
                    <h3 className="text-3xl md:text-4xl font-bold">{t.saved_names.title}</h3>
                ...
                {savedNames.length > 0 && (
                    <button 
                        onClick={() => { if(window.confirm('Clear all?')) savedNames.forEach(n => removeSavedName(n.id)) }}
                        className="text-xs font-bold uppercase opacity-40 hover:opacity-100 transition-opacity flex items-center gap-1"
                    >
            ...
            {savedNames.length === 0 ? (
                ...
                <>
                    <div className="space-y-3 mb-6 max-h-[400px] overflow-y-auto pr-1 custom-scrollbar">
                        {savedNames.map(name => (
                            <div key={name.id} className="bg-white/10 p-4 rounded-xl flex justify-between items-center group hover:bg-white/20 transition-all border border-white/50 hover:border-white/20 shadow-sm animate-fade-in">
                                <div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-center gap-3 pt-6 border-t border-white/10">
            ...
            {showImageCreator && (
                <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/60 backdrop-blur-md">
                    <div className="flex min-h-full items-center justify-center p-4">
                        <div className="relative w-full max-w-7xl transform bg-[var(--card-bg)] backdrop-blur-2xl text-left shadow-2xl transition-all rounded-3xl flex flex-col overflow-hidden border border-white/30">
                            
                            <div className="shrink-0 flex justify-between items-center px-8 py-6 bg-white/10 border-b border-white/20">
                                <h3 className="text-3xl font-bold text-[var(--text-main)] truncate">{t.saved_names.shareable_title}</h3>
                                <button onClick={() => setShowImageCreator(false)} className="text-[var(--text-main)] hover:text-black p-2 bg-white/20 hover:bg-white/40 rounded-full transition-all hover:scale-110">
                            ...
                            <div className="overflow-y-auto flex-grow p-6 sm:p-10">
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
                                     <div className="space-y-8 order-2 lg:order-1">
                                        <div className="bg-white/30 p-8 rounded-3xl shadow-sm border border-white/40">
                                            <h4 className="font-bold text-xl mb-6 flex items-center gap-2">
                                                <span className="w-8 h-8 bg-[#AA336A] text-white rounded-full flex items-center justify-center text-sm">1</span>
                                                {t.saved_names.card_step1}
                                            </h4>
                                            <Button 
                                                onClick={() => fileInputRef.current?.click()} 
                                                variant="secondary" 
                                                className="w-full !py-8 !text-xl bg-white/50 border-dashed border-2 hover:border-solid transition-all"
                                            >
                                        ...
                                        {imagePreview && (
                                            <div className="bg-white/30 p-8 rounded-3xl shadow-sm border border-white/40">
                                                <label className="block text-lg font-bold mb-4">{t.saved_names.zoom_label}</label>
                                                <input
                                                    type="range"
                                                    min="0.5"
                                                    max="3"
                                                    ...
                                                    className="w-full accent-[#AA336A] h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                                                />
                                                <p className="text-sm mt-3 italic opacity-60 text-center">{t.saved_names.zoom_hint_drag}</p>
                                            </div>
                                        )}
                                        
                                        <div className="pt-8 space-y-4">
                                            <h4 className="font-bold text-xl flex items-center gap-2">
                                                <span className="w-8 h-8 bg-[#AA336A] text-white rounded-full flex items-center justify-center text-sm">2</span>
                                                {t.saved_names.card_step2}
                                            </h4>
                                            <div className="flex flex-col gap-4">
                                                <Button onClick={handleDownloadImage} disabled={isDownloading || !imagePreview} variant="secondary" className="!py-5 !text-lg">
                                                    <DownloadIcon className="w-6 h-6 mr-2" />
                                                    {isDownloading ? t.saved_names.btn_saving : t.saved_names.btn_download}
                                                </Button>
                                                <Button onClick={handleShare} disabled={isSharing || !imagePreview} variant="primary" className="!py-5 !text-lg shadow-xl">
                                                    <ShareIcon className="w-6 h-6 mr-2" />
                                                    {isSharing ? t.saved_names.btn_preparing : t.saved_names.btn_share_card}
                                                </Button>
                                            </div>
                                            {actionError && <p className="text-red-500 text-center font-bold bg-red-50 p-4 rounded-xl">{actionError}</p>}
                                        </div>
                                     </div>
                                     ...
                                     <div className="flex flex-col items-center justify-start order-1 lg:order-2">
                                        <div className="w-full flex justify-center py-4 relative">
                                            <div 
                                                className="cursor-grab active:cursor-grabbing transform origin-top transition-transform scale-[0.6] sm:scale-[0.8] md:scale-90 lg:scale-100"
