    ...
    return (
        <Card>
            <div className="flex justify-center items-center gap-4 mb-6 relative">
                <h3 className="text-3xl font-bold text-center">{t.saved_names.title}</h3>
            ...
            {savedNames.length === 0 ? (
                ...
                <>
                    <div className="space-y-3 mb-6">
                        {savedNames.map(name => (
                            <div key={name.id} className="bg-white/10 p-4 rounded-xl flex justify-between items-center group hover:bg-white/20 transition-all border border-white/50 shadow-sm">
                                <div>
                                    <p className="font-bold text-xl">{name.name}</p>
                                    <p className="text-sm opacity-80">{name.meaning}</p>
                                </div>
                                <button onClick={() => removeSavedName(name.id)} className="p-3 text-red-400 hover:text-red-600 transition-all">
                    ...
                    <div className="flex justify-center pt-2">
                        <Button onClick={() => setShowImageCreator(true)} variant="primary" className="btn-surprise w-full sm:w-auto">
                    ...
                    <div className="flex min-h-full items-center justify-center p-4 text-center">
                        <div className="relative w-full max-w-4xl transform bg-[var(--card-bg)] backdrop-blur-xl text-left shadow-2xl transition-all rounded-2xl flex flex-col overflow-hidden border border-white/20">
                            ...
                            <div className="shrink-0 flex justify-between items-center px-8 py-6 bg-white/20 backdrop-blur-md border-b border-white/20">
                                <h3 className="text-2xl font-bold">{t.saved_names.shareable_title}</h3>
                                <button onClick={() => setShowImageCreator(false)} className="text-gray-500 hover:text-black p-2 bg-white/20 rounded-full">
                            ...
                            <div className="overflow-y-auto p-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                                     <div className="space-y-6">
                                        <h4 className="font-bold text-lg">{t.saved_names.card_step1}</h4>
                                        <Button 
                                        ...
                                        {imagePreview && (
                                            <div className="space-y-2 bg-white/10 p-4 rounded-xl">
                                                <label className="block text-sm font-bold opacity-80">{t.saved_names.zoom_label}</label>
                                                <input
                                                    type="range"
                                                    min="0.5"
                                                    max="3"
                                                ...
                                                />
                                                <p className="text-xs opacity-60 text-center">{t.saved_names.zoom_hint_drag}</p>
                                            </div>
                                        )}
                                        
                                        <div className="pt-6 border-t border-black/5 space-y-4">
                                            <h4 className="font-bold text-lg">{t.saved_names.card_step2}</h4>
                                            <Button onClick={handleDownloadImage} disabled={isDownloading || !imagePreview} variant="secondary" className="w-full">
