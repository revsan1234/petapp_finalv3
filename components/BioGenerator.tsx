...
export const BioGenerator: React.FC<BioGeneratorProps> = ({ petInfo, imageForBio, setImageForBio }) => {
    ...
    const inputStyle = { fontFamily: "'Poppins', sans-serif", fontSize: "1.2rem" };
    ...
    useEffect(() => {
        if (imageForBio) {
            setImagePreview(imageForBio); setImageZoom(1); setImagePosition({ x: 0, y: 0 }); setImageForBio(null); 
        }
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                setImagePreview(reader.result as string); setGeneratedBios([]); setSelectedBio(''); setCustomBio(''); setBioMode('ai'); setImageZoom(1); setImagePosition({ x: 0, y: 0 }); setActionError(null);
            };
    ...
    const handleGenerateBio = async () => {
        if (!petName) { setError('Please provide a name first.'); return; }
        setIsLoading(true); setError(null); setBioMode('ai');
        try {
                ...
                <div className="space-y-6">
                    <fieldset className="bg-white/5 p-6 rounded-2xl border border-white/10">
                        ...
                        <div className="space-y-4">
                            <Input id="pet-name" label={t.bio.label_name} value={petName} onChange={e => setPetName(e.target.value)} placeholder={t.placeholders.pet_name} style={inputStyle} />
                            <Select id="bio-type" label={t.generator.label_type} value={petType} onChange={e => setPetType(e.target.value as PetType)} style={inputStyle}>
                                {PET_TYPES.map(type => <option key={type} value={type}>{t.options.types[type] || type}</option>)}
                            </Select>
                            <Select id="bio-gender" label={t.bio.label_gender} value={gender} onChange={e => setGender(e.target.value as PetGender)} style={inputStyle}>
                                {PET_GENDERS.map(g => <option key={g} value={g}>{t.options.genders[g] || g}</option>)}
                            </Select>
                            <Select id="bio-personality" label={t.bio.label_vibe} value={personality} onChange={e => setPersonality(e.target.value as PetPersonality)} style={inputStyle}>
