...
import { generatePetPersonality, generatePetNames } from '../services/geminiService';
import type { QuizQuestion, PetPersonalityResult, PetInfo, GeneratedName } from '../types';
...
export const PersonalityQuiz: React.FC<PersonalityQuizProps> = ({ onQuizComplete, petInfo, setPetInfo, addSavedName, savedNames }) => {
    const { t, language } = useLanguage();
    const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
    const [answers, setAnswers] = useState<string[]>([]);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [personalityResult, setPersonalityResult] = useState<PetPersonalityResult | null>(null);
    ...
    const questions: QuizQuestion[] = [
        ...
        },
        {
            ...
            answers: [
                ...
                { text: t.quiz.a3_3, value: "Brave but sensible, not easily impressed" },
            ],
        },
    ];

    const isNameSaved = useCallback((nameId: string) => {
        ...
        if (error) {
             return (
                <div className="text-center p-4 min-h-[200px] flex flex-col justify-center items-center">
                    <p className="text-red-500 mb-4 bg-red-200/50 p-3 rounded-lg">{error}</p>
            ...
            return (
                <div className="text-center p-4 min-h-[200px] flex flex-col justify-center items-center animate-fade-in space-y-4">
                    ...
                    <div className="bg-white/20 p-4 rounded-lg">
                        <p className="text-3xl font-bold text-[#AA336A]">{personalityResult.title}</p>
                        <p className="mt-2 opacity-90 italic text-xl">"{personalityResult.description}"</p>
                    </div>

                    {isGeneratingNames ? (
                         <div className="text-center py-4">
                            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-current mx-auto"></div>
                            <p className="mt-3 opacity-80 text-xl">{t.generator.loading_text}</p>
                        </div>
                    ) : nameIdeas.length > 0 ? (
                        <div className="w-full max-w-2xl pt-4">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                {nameIdeas.map(name => (
                                    <div key={name.id} className="bg-[#494d43]/10 p-2 rounded-md font-semibold text-base flex justify-between items-center">
                                        <span>{name.name}</span>
                                        <button onClick={() => addSavedName(name)} disabled={isNameSaved(name.id)}>
                                            {isNameSaved(name.id) ? <HeartIconFilled className="w-6 h-6 text-[#AA336A]" /> : <HeartIconOutline className="w-6 h-6 text-[#AA336A]" />}
                                        </button>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="pt-4">
                        <Button onClick={handleResetAndTryAgain} variant="secondary">{t.quiz.play_again}</Button>
                    </div>
                </div>
            )
        }
        ...
        return (
            <div className="animate-fade-in w-full">
                {currentQuestionIndex === 0 && answers.length === 0 && (
                     <div className="max-w-md mx-auto space-y-4 mb-8 text-center">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <Select id="quiz-type" name="type" label={t.generator.label_type} value={petInfo.type} onChange={handleChange}>
                                {PET_TYPES.map(type => (
                                    <option key={type} value={type}>{t.options.types[type] || type}</option>
                                ))}
                            </Select>
                            <Select id="quiz-gender" name="gender" label={t.generator.label_gender} value={petInfo.gender} onChange={handleChange}>
                ...
                <div className="w-full max-w-2xl mx-auto">
                    <div className="mb-4">
                        <p className="text-center text-xl font-semibold">{currentQuestion.question}</p>
                        <div className="w-full bg-white/30 rounded-full h-4 my-4 shadow-inner">
                            <div className="bg-gradient-to-r from-pink-400 to-pink-600 h-4 rounded-full transition-all duration-300" style={{ width: `${progressPercentage}%` }}></div>
                        </div>
                    </div>
    ...
    return (
        <Card>
            <div className="flex flex-col items-center gap-2 mb-6 text-center">
                 <div className="bg-[#AA336A]/20 p-3 rounded-full mb-2">
                    <QuizIcon className="w-8 h-8 text-[#AA336A]" />
                 </div>
