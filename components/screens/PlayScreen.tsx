    ...
    return (
        <Card className="animate-fade-in">
            <div className="flex flex-col items-center text-center mb-6">
                <PetCharacter pet="bird" className="w-24 h-24 mb-4 drop-shadow-md" />
                <h2 className="text-3xl font-black text-[#5D4037]">{t.translator.title}</h2>
                <p className="opacity-80 text-lg font-bold">{t.translator.subtitle}</p>
            </div>
            <form onSubmit={handleTranslate} className="max-w-md mx-auto space-y-4">
                <Input id="trans-name" label={t.translator.label_name} value={petName} onChange={e => setPetName(e.target.value)} placeholder="e.g. Luna" />
                <Select id="trans-lang" label={t.translator.label_lang} value={targetLang} onChange={e => setTargetLang(e.target.value)}>
                    {languages.map(lang => <option key={lang} value={lang}>{lang}</option>)}
                </Select>
                <div className="flex justify-center pt-2">
                    <Button type="submit" disabled={isLoading || !petName.trim()}>{isLoading ? t.translator.btn_translating : t.translator.btn_translate}</Button>
                </div>
            </form>
            {result && !isLoading && (
                <div className="mt-8 p-6 bg-white/40 rounded-2xl text-center animate-fade-in border border-white/40 shadow-inner">
                    <p className="text-5xl font-black text-[#5D4037] mb-2">{result.translation}</p>
                    <p className="text-lg font-bold text-[#AA336A] italic">{t.translator.pronunciation}: {result.pronunciation}</p>
                </div>
            )}
        </Card>
    );
    ...
    return (
        <Card>
            <div className="text-center mb-6 flex flex-col items-center">
                <PetCharacter pet="lizard" className="w-24 h-24 mb-4 drop-shadow-md" />
                <h2 className="text-3xl font-black text-[#5D4037]">{t.horoscope.title}</h2>
                <p className="opacity-80 text-lg font-bold">{t.horoscope.subtitle}</p>
            </div>
            <div className="max-w-md mx-auto space-y-4">
                ...
                <div className="flex justify-center pt-2">
                    <Button onClick={handleGetReading} disabled={isLoading || !petName.trim()}>{isLoading ? t.horoscope.btn_reading : t.horoscope.btn_read}</Button>
                </div>
            </div>
            {reading && (
                <div className="mt-8 p-6 bg-purple-500/10 rounded-2xl animate-fade-in border border-white/20">
                    <h3 className="text-xl font-bold text-[#AA336A] mb-2">{petName}: {t.horoscope.result_title}</h3>
                    <p className="text-lg font-medium italic mb-4">"{reading.prediction}"</p>
                    <div className="bg-white/30 rounded-lg p-3 inline-block">
                        <span className="text-xs uppercase font-bold block opacity-60">{t.horoscope.lucky_item}</span>
                        <span className="font-bold text-[#5D4037]">{reading.luckyItem}</span>
                    </div>
                </div>
            )}
        </Card>
...
// --- 3. PET AGE CALCULATOR COMPONENT ---
...
const PetAgeCalculator: React.FC = () => {
    ...
    const calculate = () => {
        ...
        if (!isNaN(years)) setHumanAge(Math.floor(years * 7));
    };

    return (
        <Card>
            <div className="text-center mb-6 flex flex-col items-center">
                <PetCharacter pet="rabbit" className="w-24 h-24 mb-4 drop-shadow-md" />
                <h2 className="text-3xl font-black text-[#5D4037]">{t.age_calc.title}</h2>
                <p className="opacity-80 text-lg font-bold">{t.age_calc.subtitle}</p>
            </div>
            <div className="max-w-md mx-auto space-y-4">
                <Input id="age-input" label={t.age_calc.label_age} type="number" value={age} onChange={e => setAge(e.target.value)} placeholder="e.g. 2" />
                <div className="flex justify-center pt-2">
                    <Button onClick={calculate}>{t.age_calc.btn_calculate}</Button>
                </div>
            </div>
            {humanAge !== null && (
                <div className="mt-8 text-center animate-fade-in bg-white/30 p-6 rounded-2xl border border-white/40">
                    <p className="text-lg font-bold opacity-80 mb-1">{t.age_calc.result_prefix}</p>
                    <p className="text-6xl font-black text-[#AA336A]">{humanAge}</p>
                    <p className="text-sm font-bold uppercase tracking-widest mt-2 opacity-60">{t.age_calc.result_suffix}</p>
                </div>
            )}
        </Card>
    );
...
// --- 4. PERSONALITY QUIZ COMPONENT ---

const PersonalityQuiz: React.FC<{ 
    onQuizComplete: (res: PetPersonalityResult) => void, 
    petInfo: PetInfo, 
    setPetInfo: (info: any) => void,
    addSavedName: (n: GeneratedName) => void,
    savedNames: GeneratedName[]
}> = ({ onQuizComplete, petInfo, setPetInfo, addSavedName, savedNames }) => {
    const { t, language } = useLanguage();
    ...
    const questions = [
        { q: t.quiz.q1, a: [{t: t.quiz.a1_1, v: "Playful"}, {t: t.quiz.a1_2, v: "Calm"}, {t: t.quiz.a1_3, v: "Mischievous"}] },
        { q: t.quiz.q2, a: [{t: t.quiz.a2_1, v: "Shy"}, {t: t.quiz.a2_2, v: "Outgoing"}, {t: t.quiz.a2_3, v: "Elegant"}] },
        { q: t.quiz.q3, a: [{t: t.quiz.a3_1, v: "Chaotic"}, {t: t.quiz.a3_2, v: "Goofy"}, {t: t.quiz.a3_3, v: "Brave"}] }
    ];
    ...
    const handleAnswer = async (val: string) => {
        ...
        if (step < questions.length - 1) setStep(step + 1);
            ...
            setIsLoading(true);
            try {
                const res = await generatePetPersonality(newAns, language);
                setResult(res);
                onQuizComplete(res);
                const suggested = await generatePetNames({...petInfo, personality: res.keywords.personality, style: res.keywords.style}, language);
                setNames(suggested.slice(0, 6));
            } catch (err) { console.error(err); } finally { setIsLoading(false); }
        }
    };
    ...
    if (result) return (
        <Card className="text-center">
            <h3 className="text-xl font-bold mb-2">{t.quiz.result_title}</h3>
            <p className="text-4xl font-black text-[#AA336A] mb-2">{result.title}</p>
            <p className="italic mb-8 opacity-80 text-xl">"{result.description}"</p>
            <div className="grid grid-cols-2 gap-3 mb-8">
                {names.map(n => (
                    <div key={n.id} className="bg-white/20 p-3 rounded-xl flex justify-between items-center">
                        <span className="font-bold">{n.name}</span>
                        <button onClick={() => addSavedName(n)} className="text-[#AA336A] p-1">
                            {savedNames.some(s => s.id === n.id) ? <HeartIconFilled className="w-6 h-6" /> : <HeartIconOutline className="w-6 h-6" />}
                        </button>
                    </div>
                ))}
            </div>
            <Button onClick={() => { setStep(0); setAnswers([]); setResult(null); }} variant="secondary">{t.quiz.play_again}</Button>
        </Card>
    );
    ...
    return (
        <Card>
            <div className="text-center mb-8">
                <PetCharacter pet="hamster" className="w-24 h-24 mx-auto mb-4" />
                <h2 className="text-3xl font-black">{t.quiz.title}</h2>
                <div className="w-full bg-white/20 h-2 rounded-full mt-4 overflow-hidden shadow-inner">
                    <div className="bg-[#AA336A] h-full transition-all duration-300" style={{ width: `${(step/questions.length)*100}%` }}></div>
                </div>
            </div>
            <p className="text-xl font-bold text-center mb-6">{current.q}</p>
            <div className="space-y-3 max-w-md mx-auto">
                {current.a.map((a, i) => <Button key={i} onClick={() => handleAnswer(a.v)} variant="secondary" className="!w-full !justify-start !text-left">{a.t}</Button>)}
            </div>
        </Card>
    );
};

// --- 5. QUICK FIRE DISCOVERY COMPONENT ---

const QuickFireDiscovery: React.FC<{
    addSavedName: (n: GeneratedName) => void,
    savedNames: GeneratedName[],
    petInfo: PetInfo,
    setPetInfo: (info: any) => void
}> = ({ addSavedName, savedNames, petInfo, setPetInfo }) => {
    const { t, language } = useLanguage();
    ...
    const start = async () => {
        setIsLoading(true);
        try {
            const list = await generateQuickFireList(petInfo.style, petInfo.type, petInfo.gender, language);
            setPool(list); setWinners([]); setRound(1); setState('playing');
        } catch (err) { console.error(err); } finally { setIsLoading(false); }
    };

    const vote = (name: string) => {
        const nextWinners = [...winners, name];
    ...
    if (state === 'idle') return (
        <Card className="text-center">
            ...
            <h2 className="text-3xl font-black mb-2">{t.quick_fire.title}</h2>
            <p className="mb-8 opacity-80 text-lg font-bold">{t.quick_fire.subtitle}</p>
            <div className="max-w-xs mx-auto space-y-4 mb-6">
                <Select id="qf-style" label={t.generator.label_style} value={petInfo.style} onChange={e => setPetInfo({...petInfo, style: e.target.value})}>
                    {NAME_STYLES.map(s => <option key={s} value={s}>{s}</option>)}
                </Select>
            </div>
            <Button onClick={start} disabled={isLoading}>{isLoading ? t.quick_fire.btn_preparing : t.quick_fire.btn_start}</Button>
        </Card>
    );

    if (state === 'playing') {
        const p1 = pool[(round - 1) * 2];
        const p2 = pool[(round - 1) * 2 + 1];
        return (
            <Card className="text-center">
                <div className="flex justify-between items-center mb-8 px-4">
                    <span className="font-bold opacity-60">{t.quick_fire.mode}</span>
                    <span className="bg-[#AA336A] text-white px-3 py-1 rounded-full text-sm font-bold">{round}/10</span>
                </div>
    ...
    return (
        <Card className="text-center">
            <h3 className="text-3xl font-black text-[#AA336A] mb-4">{t.quick_fire.winner_title}</h3>
            ...
            <Button onClick={() => setState('idle')} variant="secondary">{t.quick_fire.btn_play_again}</Button>
        </Card>
    );
};
    ...
    return (
        <Card className="flex flex-col h-[600px] max-w-full">
            <div className="flex items-center justify-between border-b border-black/10 pb-4 mb-4">
                ...
                <button onClick={() => { setMessages([]); localStorage.removeItem('pet_chat_history'); }} className="text-xs font-bold opacity-40 uppercase">{t.expert.btn_clear}</button>
            </div>
...
export const PlayScreen: React.FC<PlayScreenProps> = ({ onQuizComplete, savedNames, addSavedName, petInfo, setPetInfo, goHome }) => {
  ...
  if (mode === 'menu') {
    const menuItems: { id: PlayMode, title: string, desc: string, pet: PetKind }[] = [
      ...
      { id: 'battle', title: t.quick_fire.title, desc: t.quick_fire.subtitle, pet: 'cat' },
    ...
    return (
      <div className="relative min-h-screen">
        ...
        <main className="px-4 pb-32 max-w-4xl mx-auto w-full flex flex-col gap-6 animate-fade-in mt-2">
            <div className="-mt-4"><BackToHomeButton onClick={goHome} /></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {menuItems.map(item => (
                    <button key={item.id} onClick={() => setMode(item.id)} className="bg-[var(--card-bg)] backdrop-blur-md rounded-[2rem] p-6 shadow-md border-2 border-white/40 flex flex-col items-center text-center gap-3 transition-all hover:scale-[1.02] hover:shadow-xl active:scale-95 group">
                        <PetCharacter pet={item.pet} className="w-20 h-20 mb-2 transform group-hover:-translate-y-1 transition-transform" />
                        <h3 className="text-2xl font-black text-[var(--text-main)] group-hover:text-[#AA336A]">{item.title}</h3>
                        <p className="text-sm font-bold opacity-80 line-clamp-2">{item.desc}</p>
                    </button>
                ))}
            </div>
        </main>
  ...
  return (
    <div className="relative min-h-screen">
        <Header leftPet="hamster" rightPet="bird" onLogoClick={() => setMode('menu')} />
        <main className="px-4 pb-32 max-w-xl mx-auto w-full animate-fade-in mt-4">
             <div className="-mt-4 flex gap-3 mb-6">
                <button onClick={() => setMode('menu')} className="flex items-center gap-2 text-white hover:scale-105 transition-all bg-white/20 px-4 py-2 rounded-full backdrop-blur-md font-bold text-sm w-fit shadow-sm">
            ...
            {mode === 'horoscope' && <PetHoroscope />}
            {mode === 'calculator' && <PetAgeCalculator />}
            {mode === 'expert' && <Consultant />}
        </main>
    </div>
  );
};
